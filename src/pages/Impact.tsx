  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ position: 'absolute', bottom: -60, left: -60, width: 320, height: 320, borderRadius: '50%', background: 'radial-gradient(circle, rgba(46,168,102,0.12), transparent 65%)', pointerEvents: 'none' }} />
        <FadeIn>
          ...
          <p style={{ fontSize: 18, color: 'rgba(239,230,216,0.7)', maxWidth: 560, margin: '0 auto' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <FadeIn>
            <div style={{ textAlign: 'center', marginBottom: 60 }}>
              ...
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 42, fontWeight: 800, color: '#0a1f14', marginBottom: 16 }}>
                {t('pages.impact.sdgTitle')}
              </h2>
              <p style={{ color: '#5a7062', fontSize: 16, lineHeight: 1.8, maxWidth: 560, margin: '0 auto' }}>
          ...
          <FadeIn delay={80}>
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
              {SDG_ALIGNMENT.map((sdg, idx) => (
                <div
                  key={sdg.num}
                  style={{ background: sdg.color, borderRadius: 16, padding: '22px 26px', minWidth: 130, textAlign: 'center', boxShadow: `0 8px 28px ${sdg.color}44` }}
                >
                  <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 30, fontWeight: 800, color: '#fff', marginBottom: 6 }}>
                    {sdg.num}
                  </div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.85)', letterSpacing: 0.5, textTransform: 'uppercase' }}>
                    {sdg.label}
                  </div>
                </div>
              ))}
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <FadeIn>
            <div style={{ textAlign: 'center', marginBottom: 64 }}>
              ...
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 42, fontWeight: 800, color: '#0a1f14' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
            {IMPACT_STORIES.map((story, idx) => (
              <FadeIn key={story.title} delay={idx * 100}>
                <div style={{ background: '#fff', borderRadius: 24, overflow: 'hidden', boxShadow: '0 8px 36px rgba(14,99,61,0.07)' }}>
                  <div style={{ height: 160, background: story.gradient, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontSize: 60 }}>{story.emoji}</span>
                  </div>
                  <div style={{ padding: 28 }}>
                    <div style={{ fontSize: 11, color: '#9aaa9e', marginBottom: 8, letterSpacing: 0.5 }}>{story.region}</div>
                    <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 800, color: '#0a1f14', marginBottom: 14 }}>{story.title}</h3>
                    <p style={{ fontSize: 14, color: '#6b7c6e', lineHeight: 1.75, marginBottom: 20 }}>{story.body}</p>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {story.sdgs.map(tag => (
                        <span key={tag} style={{ background: 'rgba(14,99,61,0.08)', color: '#0e633d', padding: '4px 12px', borderRadius: 50, fontSize: 10, fontWeight: 700, letterSpacing: 0.4 }}>{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
